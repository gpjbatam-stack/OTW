...
function renderPricing(){
   ...
   * dapat mempunyai semantic field yang berbeda. Review selalu menjadikan
   * totalPrice sebagai angka otoritatif, kemudian menambahkan add-on SEKALI.
   */
  ...
  const authoritativeFlightTotal = Number(
    flightPricing.totalPrice ??
    flight?.displayPrice ??
    0
  ) || 0;

  // Derive fare component from authoritative total for a mathematically exact breakdown.
  const ticketFare = Math.max(0, authoritativeFlightTotal - serviceFee);

  pricing.ticketFare = ticketFare;
  pricing.serviceFee = serviceFee;

  // Add-ons are recomputed from the actual selected rows in renderAddons().
  pricing.grandTotal = authoritativeFlightTotal + pricing.addonsTotal;

  $("#ticketPrice").textContent = rupiah(authoritativeFlightTotal);
...
function finalConfirm(){
  const payload={
    ...
    addons,
    pricing:{
      flightTotal:Number(flightPricing?.totalPrice||flight?.displayPrice||0),
