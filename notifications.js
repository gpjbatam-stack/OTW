...
let user=null,notifications=[],activeFilter="all",channel=null,userSettings=null,loadTimer=null,loading=false;
...
async function load(){if(loading)return;loading=true;show("loading");try{if(!user&&!(await ensureUser()))return;try{await supabase.rpc("sync_my_letsgo_notifications")}catch(e){console.warn("[LetsGo notification sync]",e)}const {data,error}=await supabase.from("notifications").select("id,user_id,order_id,order_code,title,message,type,is_read,created_at").eq("user_id",user.id).order("created_at",{ascending:false}).limit(100);if(error)throw error;userSettings=readUserSettings(user.id);notifications=(data||[]).filter(n=>isNotificationEnabled(n,userSettings));render()}catch(e){console.error(e);$("#errorMessage").textContent=e?.message||"Silakan coba lagi.";show("error")}finally{loading=false}}
...
function scheduleLoad(delay=140){clearTimeout(loadTimer);loadTimer=setTimeout(()=>load(),delay)}
async function startRealtime(){if(!user)return;if(channel)await supabase.removeChannel(channel);channel=supabase.channel(`letsgo-notifications-${user.id}`).on("postgres_changes",{event:"*",schema:"public",table:"notifications",filter:`user_id=eq.${user.id}`},()=>scheduleLoad()).subscribe()}
...
document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible")scheduleLoad(0)});window.addEventListener("pagehide",()=>{if(channel)supabase.removeChannel(channel)});
